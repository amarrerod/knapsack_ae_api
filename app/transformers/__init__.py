...
from app.models import KP, KPCollection, read_instance

async def decode(x0: float, x1: float):
    variables = __decode_instances((x0, x1))
    return KP(size=len(variables), variables=variables)

async def encode(collection: KPCollection):
    variables = [i.variables for i in collection.instances]
    return (__encode_instances(variables), 200)
